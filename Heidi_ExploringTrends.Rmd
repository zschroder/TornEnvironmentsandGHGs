...
N2O Annual Trend Analysis Chart
```{r}
ggplot(AnnGHG, aes(x=year, y=N2O_ppb))+
...
SF6 Annual Trend Analysis Chart
```{r}
...
Combined annual averages (trying to create a 4x6 layout of the annual trends from above)
```{r}
...
grid.arrange(CO2_avg, CH4_avg, N2O_avg, SF6_avg, O3_avg, AGGI_avg,  ncol=3 , nrow=2, top ="Annual Averages of Greenhouse Gases")


```

Total annual average of GHG (Finding this for the table within Data and Methods Section B)
```{r}

CO2_Average <- mean(AnnGHG$CO2_ppm)
CH4_Average <- mean(AnnGHG$CH4_ppb)
O3_Average <- mean(AnnGHG$O3_DU)
AGGI_Average<-mean(AnnGHG$AGGI)
...
4x4 Plot of GHG Avg/year (focused on main four)
```{r}
...
4x4 Plot of 10-year Averages(main four)
```{r}
grid.arrange(AGGI, O3, CH4, CO2, nrow=2, ncol=2, top="Ten year Averages of Greenhouse Gas Emissions")
...
ggplot(T_yr, aes(x=Year, y=T_Year))+
  geom_bar(stat="identity", position = "stack", fill="plum3", linewidth=0.9)+
  #stat_smooth(aes(x=Year, y=T_Year), method = "lm", formula = y~x, geom="smooth", color="black", linewidth=1.5)+
  ggtitle("Total Tornado Outbreaks Per Year")+
  theme_bw()+
  theme(plot.title=element_text(hjust=0.5))+
  xlab("Year")+
...
#ggplot(T_yr, aes(x=Year, y=T_Year))+
 # geom_bar(stat="identity",position = "stack",  fill="plum3", width=0.9)+
  ...
  #scale_x_continuous(expand=c(0,0), limits=c(1994,2023), breaks=c(seq(1994, 2023, 4)))+
  #scale_y_continuous(expand=c(0,0), limits=c(20,50), breaks=c(seq(20,50,5)))

```
 
```{r}
ggplot(T_yr, CO2_1avg, aes(x+T_Year, y=CO2_1year))+
  geom_point(stat="identity", position = "stack", fill="orchid3")+
  stat_smooth(method = "lm", formula = y~x, geom = "smooth", color="black", linewidth=1.5)+
  theme_bw()+
  xlab("Total Outbreaks per Year")+
  ylab("CO2 Concenrations (ppm)")
...
coordinates <- BigDays.sf.dfT[,"Lat","Lon"] 
...
tm_shape(stateBorders, projection = "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m   +no_defs")+
  ...
  tm_scale_bar(width = .3, 
               ...
               lwd = 1.75, 
               color.dark = "gray")+
  tm_layout(attr.position = c("left","bottom"), inner.margins = c(.1, .1, .1, .1))+
