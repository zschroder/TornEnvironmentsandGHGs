...
Seasons <- data.frame(seasons =c("DJF", "MAM", "JJA","SON"), Total_Clusters = c(DJF, MAM, JJA, SON))
...
```{r}
GHG<-BigDays.sfdfT%>%
  dplyr::select(Year, CO2_10year,CH4_10year, O3_10year, AGGI_10year)

Torn<-BigDays.sfdfT%>%
  group_by(Year)%>%
  summarise(TotClus_yr=n(),
            TotCas_yr = sum(GroupDayCas))
```
...
colnames(TenYearCor) = c("Total Torn.","Total Cas.","CO2", "CH4", "O3", "AGGI")
...
#Total Clusters and GHG
t.test(Torn$TotClus_yr, BigDays.sfdfT$CO2_10year)
t.test(Torn$TotClus_yr, BigDays.sfdfT$CH4_10year)
t.test(Torn$TotClus_yr, BigDays.sfdfT$O3_10year)
t.test(Torn$TotClus_yr, BigDays.sfdfT$AGGI_10year)


#Total Casualties and GHG
t.test(Torn$TotCas_yr, BigDays.sfdfT$CO2_10year)
t.test(Torn$TotCas_yr, BigDays.sfdfT$CH4_10year)
t.test(Torn$TotCas_yr, BigDays.sfdfT$O3_10year)
t.test(Torn$TotCas_yr, BigDays.sfdfT$AGGI_10year)
...
AGGI_10<-ggplot()+
  ...
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 1.4), 
                     breaks = seq(0,1.4,.2), 
                     sec.axis = sec_axis(~.*50, name = "Total Tornado Outbreaks")) 
```

```{r}
grid.arrange(CO2_10, CH4_10, O3_10, AGGI_10, nrow=2, top ="Decadal Average Emissions & Total Clusters  per Year")
```
