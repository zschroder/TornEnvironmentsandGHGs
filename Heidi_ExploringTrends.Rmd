...
############################
```{r}
TotClus<-BigDays.sfdfT%>%
  group_by(Year)%>%
  summarise(TotClusYr = n())
...
#creating data frame 
AnnualYearCor <-data.frame(TotClus,TotCas_yr, AnnualYearCor)
#AnnualYearCor <- AnnualYearCor[1:6]
...
TenYearCor <- BigDays.sfdfT%>%
  ...
  #selecting variables
  dplyr::select(TotCas_yr, GroupDayCas,GroupDayFat,
                CO2_10year,
                CH4_10year, 
                O3_10year,
                AGGI_10year)
...
#formatting matrix
corrplot(cordat, method ="number",type= "full", tl.col = "black")
```
