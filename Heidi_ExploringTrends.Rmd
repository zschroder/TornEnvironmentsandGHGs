...
#install.packages("gridExtra") I have not installed this package yet. Waiting to see if this is the one I need to create the 4x6 layout fo rthe 10 year averages
```
...
Combined 10-year averages
```{r}

#Might need to use the grid.arrange funcation from the gridExtra package. 
...
ggplot(T_yr,aes(x=Year, y=T_Year))+
  geom_bar(stat = "identity",position="stack", fill="plum", width = 0.9)+
  stat_smooth(method = "lm", formula = y~x, geom="smooth", color="black", linewidth=1.5)+
  theme_bw()+
  xlab("Year")+
...
ggplot(T_yr)+
  geom_line(aes(x=Year, y=T_Year), color="plum", linewidth=1)+
  theme_bw()+
  xlab("Year")+
  ylab("Number of Tornado Outbreaks per Year")+
  scale_x_continuous(expand = c(0,0),limits=c(1994,2022), breaks=seq(1994,2022,2))+
