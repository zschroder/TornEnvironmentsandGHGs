...
##ERROR in Formatting below: Invalid Symbol Colors
...
##Annual Change in Tornado Outbreaks Casualties
...
test_10yr <- BigDays.sfdfT %>%
  ...
  summarize(Totout_yr =n(), #Total Number of outbreaks
            ...
            #Emissions for 10 years
            CO2_10yr = CO2_10year,
            CH4_10yr = CH4_10year,
            O3_10yr = O3_10year,
            AGGI_10yr = AGGI_10year) %>%
...
**Scatter Plot**
```{r}
#need to pull each individual 10 year average for CO2 should be at least 3
##ERROR Error in if (BigDays.sfdfT$Year[i] == 1994) { : missing value where TRUE/FALSE needed
for (i in BigDays.sfdfT$Year){
  if (BigDays.sfdfT$Year[i]==1994){
    CO2_94 = CO2_10year
  }
  else if (BigDays.sfdfT$Year[i] == 2004){
    CO2_04 = CO2_10year
  }
  else if (BigDays.sfdfT$Year[i] == 2014){
    CO2_14 = CO2_10year
  }
}
#need to pull average outbreak size range (I'm assuming nT is the size)

```

```{r}
ggplot(test_10yr, aes(x = avgTorn, y=CO2_10yr))+
  geom_point()
