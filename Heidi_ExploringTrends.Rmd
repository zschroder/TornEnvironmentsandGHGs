...
This code does not do anything and is unnecessary: 
#AnnGHG%>%
 # group_by(year)%>%
 # select("CO2_ppm")
...
Thiscode does not do anything and is unnecessary: 
#AnnGHG%>%
#  group_by(year)%>%
#  select("CH4_ppb")
```{r}  
...
*Here is the code to rename a column in R: *
```{r}
...
*Do not need to read this in again*
...
#2020s 
#Why are all of these N/A?
CO2_20avg = mean(AnnGHG[seq(62,65),2]) 
CH4_20avg = mean(AnnGHG[seq(62,65),3])
N2O_20avg = mean(AnnGHG[seq(62,65),4]) 
SF6_20avg = mean(AnnGHG[seq(62,65),5]) 
O3_20avg = mean(AnnGHG[seq(62,65),6]) 
AGGI_20avg = mean(AnnGHG[seq(62,65),7]) 
...
#c(CO2_90avg, CO2_00avg, CO2_10avg, CO2_20avg)

# AnnGHG%>%
#   nineties <- filter(AnnGHG, year %in% c(1990,1999))
#   thousands <-filter(AnnGHG, year %in% c(2000,2009))
# 
# print(nineties)
```

*Example for plotting CO2:*
```{r}
...
#   scale_y_continuous(expand=c(0,0), limits=c(0,2), breaks=c(seq(0,2,0.1)))
```
Cluster: >= 10 tornadoes on 1 convective day
