---
title: Exploring Trends in Tornado Outbreaks and Greenhouse Gas Emissions between
  1994 and 2022
author: "Dr. Searcy"
date: "2024-02-28"
output: html_document
editor_options: 
  chunk_output_type: console
---

“Exploring trends in tornado outbreak environments and greenhouse gas concentrations” 

`Pursue publication:` International Journal of Climatology
                      https://mc.manuscriptcentral.com/joc 

**Make sure to follow the Author Resources for publication in the Journal. This includes page formatting, section headers, figure format, table format, figure/table location in manuscript, etc.**

```{r}
library("dplyr")
library("ggplot2")
library("tmap")
library("USAboundaries")
library("sf")
library("grid")
library("gridExtra")
library("corrplot")
```

Projections you may need: 
```{r}
merc <- "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
US_LCC <- "+proj=lcc +lat_1=33 +lat_2=45 +lat_0=39 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs"
```

Load the dataset into the R workspace.
```{r}
load("TornadoOutbreaks.RData")
```

```{r}
#getting State Borders for contiguous United States
sts <- state.name[!state.name %in% c("Alaska", "Hawaii")] #Removes AK and HI from list of states
stateBorders <- us_states(states = sts) 
```

**Florida Tornadoes in the data.**
```{r}
#Pulling all tornado reports for Florida
Florida<-All_Tornadoes%>%
  dplyr::filter(st=="FL")

#filter state borders for only Florida
FL_map <- stateBorders%>%
  dplyr::filter(state_abbr=="FL")
```

Adjusting the color palette for "Reds":
```{r}
cr <- RColorBrewer::brewer.pal(9, "Reds")
cr <- cr[-c(1:3)]
```

Create a map of all Florida tornadoes in the SPC data set from 1994 to 2022.
```{r}
tm_shape(FL_map, is.master = TRUE, projection = merc) + 
  tm_polygons(alpha = 0, 
              border.col = "black", 
              lwd = 1.5) +
tm_shape(Florida) +
  tm_bubbles(size = 0.15,
             col = "mag",
             breaks=seq(0,5,by=1),
             #border.lwd= 0.4,
             #border.col = "mag",
             border.alpha = 0,
             palette = cr,
            labels=c("EF0", "EF1","EF2", "EF3", "EF4"),
            title.col="Magnitude") +
  tm_compass(text.size = 1.5, 
             lwd = 2, 
             color.dark = "gray") +
  tm_scale_bar(width = .3, 
               text.size = 1, 
               lwd = 1.75,  
               color.dark = "gray") +
  tm_layout(title="Florida Tornadoes\n     1994 - 2022", 
            title.position=c("center", "top"), 
            title.size = 1.5,
            legend.show = TRUE,
            legend.bg.color = "white",
            legend.height = 1.25,
            legend.position = c(0.02,0.01),
            legend.text.size = 1.25, 
            legend.title.size = 1.5,
            attr.position = c("right","bottom"), 
            inner.margins = c(.1, .05, .11, .1))
```
`FIG_FLTornData`


## Create a figure (Map) with multiple clusters on a single day

`If you are wanting to find a day that has more than one cluster, do this: `
```{r}
x <- BigDays.sfdfT %>% # Cluster Data
  group_by(cDate) %>% # Group by the convective days to see if there are duplicates.
  summarize(num_of_clusters = n()) %>% # Count the number of clusters on each cDate.
  arrange(desc(num_of_clusters)) # Arrange the number of clusters column in descending order.
```
We will use the June 4, 1999 outbreak as an example of a single big day with multiple clusters. This results in two separate reports/clusters reported in the data set. 

Extract the tornado data for June 4, 1999 outbreak:
```{r}
Jun04 <-  BigDays.sfdfT %>%
  filter(cDate == "1999-06-04")

Jun04cent <-BigDayCentroids.df %>%
  filter(ID %in% Jun04$ID)

Jun04Torns <- BigDayTornadoes%>%
  filter(ID %in% Jun04$ID)  
```
  
**Plotting  June 4, 1999 outbreak data.**
```{r}
tm_shape(stateBorders) +
  tm_borders(col ="gray70", 
             alpha = 1) +
  tm_compass(text.size = 1.25, size = 3,
             lwd = 2, 
             color.dark = "gray70") +
  tm_scale_bar(width = .2, 
               text.size = 1.25, 
               lwd = 1.75,  
               color.dark = "gray70") +
  tm_layout(attr.position = c("right","bottom"), 
            inner.margins = c(.1, .05, .1, .05)) +
tm_shape(Jun04, 
         is.master = TRUE, 
         projection = merc) + 
  tm_polygons(alpha = 0, 
              border.col = "black", 
              lwd = 1.5) +
tm_shape(Jun04Torns) +
  tm_bubbles(alpha = 0.7,
             size = 1, 
             col="ID", 
             breaks=seq(0,5,by=1),
             #border.lwd= 0.4,
             #border.col = "black",
             border.alpha = 0,
             palette = c("blue", "red")) +
  tm_layout(title="June 4, 1999", 
            title.position = c("center", "top"), 
            title.size = 1.75,
            legend.show = FALSE) +
tm_shape(Jun04cent) +
  tm_symbols(size = 1.25, 
             col="black", 
             shape=24)
```
`FIG_MultiClusDay`

**Redoing plot to new method in updated file**
```{r}
BG<-BigDays.sfdfT%>%
  dplyr::filter(ID=="202112156918")

BGcent <-BigDayCentroids.df%>%
  dplyr::filter(ID=="202112156918")

BGTorns <- BigDayTornadoes%>%
  dplyr::filter(ID=="202112156918") 

```

```{r}
tm_shape(stateBorders, 
         projection = merc) +
  tm_borders(col ="gray70", 
             alpha = 1) +
  tm_compass(text.size = 1.25, size = 3,
             lwd = 2, 
             color.dark = "gray70") +
  tm_scale_bar(width = .25, 
               text.size = 1.25, 
               lwd = 1.75,  
               color.dark = "gray70") +
  tm_layout(attr.position = c("right","bottom"), 
            inner.margins = c(.15, .15, .15, .15)) +
tm_shape(BG, 
         is.master = TRUE, 
         projection = merc) + 
  tm_polygons(alpha = 0, 
              border.col = "black", 
              lwd = 1.5) +
tm_shape(BGTorns,
         projection = merc) +
  tm_bubbles(alpha = 0.7,
             size = 0.8, 
             col="mag", 
             breaks = seq(0,3,by=1),
             border.alpha = 0,
             palette = cr,
             labels = c("EF0", "EF1","EF2"),
             title.col="Magnitude")+
  tm_layout(title= "December 15, 2021\n    125 tornadoes", 
            title.position = c("center", "top"), 
            title.size = 1.75,
            legend.show = TRUE,
            legend.position = c("left", "bottom"), 
            legend.height = 2,
            legend.text.size = 1.25, 
            legend.title.size = 1.5,) +
tm_shape(BGcent,
         is.master=TRUE,
         projection = merc) +
  tm_symbols(size = 1.25, 
             col="black", 
             shape=24)
  
```

Grouping Total Number of Outbreaks per Year
```{r}
Totout_mon <- BigDays.sfdfT%>% 
  group_by(Month)%>%
  summarize(Totout_Monthly = n())

DJF<-sum(Totout_mon$Totout_Monthly[12],Totout_mon$Totout_Monthly[1],Totout_mon$Totout_Monthly[2])
MAM<-sum(Totout_mon$Totout_Monthly[3],Totout_mon$Totout_Monthly[4],Totout_mon$Totout_Monthly[5])
JJA<-sum(Totout_mon$Totout_Monthly[6],Totout_mon$Totout_Monthly[7],Totout_mon$Totout_Monthly[8])
SON<-sum(Totout_mon$Totout_Monthly[9],Totout_mon$Totout_Monthly[10],Totout_mon$Totout_Monthly[11])
Seasons <- data.frame(seasons =c("DJF", "MAM", "JJA","SON"))
Seasons$Total_Clusters<- c(DJF, MAM, JJA, SON)

Seasons
```

Graphing Total Outbreaks per Year
```{r}
Seasons$seasons <- factor(Seasons$seasons, levels = Seasons$seasons)
test <- BigDays.sfdfT %>%
  group_by(Month) %>%
  summarize(numClus = n())

#Plotting Outbreaks
ggplot(test,
       aes(x=Month, y=numClus))+
  geom_bar(stat="identity", 
            fill="red3", 
            linewidth=1)+
  scale_x_discrete(labels = month.abb) +  
  coord_polar(start =0) +
  theme_bw() +
  theme(plot.title=element_text(size = 18, hjust=0.5), 
        plot.margin=margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x=element_text(size = 14, angle=45, hjust=1),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16))+
  xlab("Month")+
  ylab("Total Clusters")+
  scale_y_continuous(expand = c(0,0),
                     limits=c(0,300), 
                     breaks=seq(0,300,50))

```

```{r}
( p0 <- ggplot(DLBSByMonth, aes(x = Mo, y = avgDLBS)) +
  geom_bar(stat = "identity", fill = "gray70") +
 scale_x_continuous(breaks = seq(1, 12, 1), labels = month.abb) +
  scale_y_continuous(limits = c(0, 2)) +
  coord_polar(start =0) +
  labs(x = "Month", y = expression(paste("Normalized Averages of DLBS [m ", s^-1, "]"))) +
  #ggtitle("a") +
  theme_minimal() )
```
`FIG_ClusbySeason`

##################
## GHG GML Data ##
##################
```{r}
AnnGHG <- read.csv("AnnualAverages_GHG.csv")
```

CO2: "blue"
CH4: "purple3"
SF6: "red4"
N2O: "deeppink"
O3: "goldenrod"
AGGI: "green4"


Carbon Dioxide Emissions by Year
```{r}
p1 <- ggplot(AnnGHG, 
      aes(x= year, y=CO2_ppm))+
   geom_line(stat="identity", 
             color = "blue",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Carbon Dioxide (in ppm)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(330, 420), 
                     breaks = seq(330, 420, 10)) 
p1
```

Methane Emissions by Year
```{r}
p2 <- ggplot(AnnGHG, 
      aes(x= year, y=CH4_ppb))+
   geom_line(stat="identity", 
             color = "purple3",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Methane (in ppb)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(1550, 1950), 
                     breaks = seq(1550, 1950, 50)) 
p2
```

```{r}
p3 <- ggplot(AnnGHG, 
      aes(x= year, y=SF6_ppt))+
   geom_line(stat="identity", 
             color = "red2",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Sulfur Hexafluoride (in ppt)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(0, 12), 
                     breaks = seq(0, 12, 2)) 

p3
```

```{r}
p4 <- ggplot(AnnGHG, 
      aes(x= year, y=N2O_ppb))+
   geom_line(stat="identity", 
             color = "deeppink",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Nitrous Oxide (in ppb)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(300, 340), 
                     breaks = seq(300, 340, 5)) 

p4
```

Ozone Emissions by Year
```{r}
p5 <- ggplot(AnnGHG, 
      aes(x= year, y=O3_DU))+
   geom_line(stat="identity", 
             color = "goldenrod",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Ozone (in DU)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(90, 230), 
                     breaks = seq(90, 230, 10)) 
p5
```

Annual Greenhouse Gas Index by Year
```{r}
p6 <- ggplot(AnnGHG, 
      aes(x= year, y = AGGI))+
   geom_line(stat="identity", 
             color = "green4",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Annual Greenhouse Gas Index (AGGI)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1979, 2022), 
                     breaks = seq(1979, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(0.7, 1.5), 
                     breaks = seq(0.7, 1.5, 0.1)) 
p6
```

4x4 Grid Annual Averages
```{r}
grid.arrange(p1, p2, p3, p4, p5, p6,
             nrow = 3,
             ncol = 2, 
             top = textGrob(" ", 
                            gp=gpar(fontsize = 20)))
```

`FIG_GHGConcentrations`

## Outbreak Density by County:

**getting county data**
```{r}
sts <- state.name[!state.name %in% c("Alaska", "Hawaii")] 
countyBorders <- us_counties() 

countyBorders <- countyBorders[,-13]

#Remove AK and HI
countyBorders <- countyBorders %>%
  filter(!state_abbr %in% c("AK", "HI", "PR"))

#Set the coordinate reference system:
countyBorders <- st_transform(countyBorders, 
                            crs = st_crs(stateBorders))
BigDays.sfdfT <- st_transform(BigDays.sfdfT, 
                            crs = st_crs(stateBorders))
```

**counting amount of times each outbreak crosses/interesects a county in Contiguous United States**
```{r}
gI <- st_intersects(BigDays.sfdfT, 
                    countyBorders, 
                    sparse = FALSE)
colnames(gI) <- countyBorders$name
countyBorders$GroupDayCount <- colSums(gI)
```

**creating map of outbreak density in each U.S.county. Fill by the `GroupDayCount` column.**
```{r}
tm_shape(countyBorders) +
   tm_fill("GroupDayCount",
           title = "",
           palette = "Reds", 
           breaks = c(0, 25, 50, 75, 100, 125)) +
  tm_borders(col = "gray50", 
             lwd = 1,
             alpha = .1) +
  tm_compass(text.size = 1, 
             size = 3,
             lwd = 2, 
             color.dark = "gray70") +
  tm_scale_bar(width = .2, 
               text.size = 1, 
               lwd = 1.75,  
               color.dark = "gray70") +
  tm_layout(legend.position = c(0.85, 0.0),
            attr.position = c("left", "bottom"),
            legend.frame = FALSE,
            title = "Tornado Clusters by County 1994-2022",
            title.size = 1.5,
            title.position = c(0.25, 0.95),
            inner.margins = c(.1, .01, .1, .01),
            legend.width = -0.45, 
            legend.height = -0.2, 
            legend.title.size = 1.25,
            legend.text.size = 1,
            legend.bg.alpha = 1) +
tm_shape(stateBorders, 
         is.master = TRUE,
         projection = merc) +
  tm_borders(col ="gray50", 
             alpha = 1) 
```
`FIG_OutbreakDens`

## Tornado Outbreaks by Year

Grouping Total Number of Outbreaks per Year
```{r}
Totout_yr <- BigDays.sfdfT %>% 
  group_by(Year) %>%
  summarize(Totout_Year = n())
```

Graphing Total Outbreaks per Year
```{r}
#Plotting Outbreaks
ggplot(Totout_yr,
       aes(x=Year, y=Totout_Year))+
  geom_line(stat="identity", 
            color="red3", 
            linewidth=1)+
  stat_smooth(method = "lm", 
              formula = y~x, 
              geom="smooth", 
              color="black", 
              linewidth=1, 
              se = FALSE) +
  ggtitle("Total Clusters per Year") +
  theme_bw() +
  theme(plot.title = element_text(hjust=0.5,
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year")+
  ylab("Total Clusters") +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1994,2022),
                     breaks=seq(1994,2022,2)) +
  scale_y_continuous(expand = c(0,0),
                     limits=c(20,50), 
                     breaks=seq(20,50,5))
```
`FIG_ClusbyYear`

## Tornado Casualties by Year

Grouping Total Number of Outbreaks per Year
```{r}
TotCas_yr <- BigDays.sfdfT%>% 
  group_by(Year)%>%
  summarize(TotalCas_yr = sum(GroupDayCas))
```

Graphing Total Outbreak Casualties per Year
```{r}
ggplot(TotCas_yr,
       aes(x = Year, 
           y = TotalCas_yr)) +
  geom_line(stat = "identity", 
            color = "orange2", 
            linewidth = 1) +
  stat_smooth(method = "lm", 
              formula = y~x, 
              geom = "smooth", 
              color = "black", 
              linewidth = 1,
              se = FALSE) +
  ggtitle("Cluster Casualties per Year")+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, 
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year") +
  ylab("Total Casualties") +
  scale_x_continuous(expand = c(0,0),
                     limits = c(1994, 2022),
                     breaks = seq(1994, 2022, 2)) +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,6000), 
                     breaks = seq(0,6000,500))
```

`FIG_ClusCas`

############################
## 10-Year  Change in GHG ##
############################

CO2 10-year Averages
```{r}
p1 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y= CO2_10year)) +
   geom_line(stat="identity", 
             color = "blue",
             linewidth = 1.5) +
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Carbon Dioxide (in ppm)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(340, 420), 
                     breaks = seq(340, 420, 10)) 
p1
```


Methane Emissions by Year
```{r}
p2 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y= CH4_10year)) +
   geom_line(stat="identity", 
             color = "purple3",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Methane (in ppb)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(1600, 1950), 
                     breaks = seq(1600, 1950, 50)) 
p2
```

```{r}
p3 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y=SF6_10year))+
   geom_line(stat="identity", 
             color = "red2",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Sulfur Hexafluoride (in ppt)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(0, 12), 
                     breaks = seq(0, 12, 2)) 

p3
```

```{r}
p4 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y=N2O_10year))+
   geom_line(stat="identity", 
             color = "deeppink",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Nitrous Oxide (in ppb)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(300, 340), 
                     breaks = seq(300, 340, 5)) 

p4
```
Ozone Emissions by Year
```{r}
p5 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y= O3_10year)) +
   geom_line(stat="identity", 
             color = "goldenrod",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Ozone (in DU)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(90, 180), 
                     breaks = seq(90, 180, 10)) 

p5
```

Annual Greenhouse Gas Index by Year
```{r}
p6 <- ggplot(BigDays.sfdfT, 
      aes(x= Year, y= AGGI_10year)) +
   geom_line(stat="identity", 
             color = "green4",
             linewidth = 1.5)+
   theme_bw() +
   theme(axis.text.x = element_text(angle=45, 
                                    hjust=1, 
                                    size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y = element_text(size = 16)) +
   ylab(expression("Annual Greenhouse Gas Index (AGGI)")) +
   xlab("Year") +
   scale_x_continuous(expand = c(0, 0), 
                     limits = c(1994, 2022), 
                     breaks = seq(1994, 2022, 3)) + 
   scale_y_continuous(expand = c(0,0), 
                     limits = c(0.85, 1.5), 
                     breaks = seq(0.85, 1.5, 0.1)) 
p6
```

```{r}
grid.arrange(p1, p2, p3, p4, p5, p6,
             nrow=3,
             ncol=2, 
             top = textGrob(" ", 
                            gp=gpar(fontsize = 20)))
```
`FIG_DecadalGHG`
**10-Year Correlation Matrix**
```{r}
TenYearCor <- BigDays.sfdfT%>%
dplyr::select(nT, 
              GroupDayCas,
              CO2_10year,
              CH4_10year, 
              SF6_10year,
              N2O_10year,
              O3_10year,
              AGGI_10year)

#creating column names
colnames(TenYearCor) = c("Total Tornades","Total Casualties","CO2", "CH4", "SF6", "N2O", "O3", "AGGI")

#creating data frame
TenYearCor <- as.data.frame(TenYearCor)
TenYearCor <- TenYearCor[1:8]

#running correlation test
cordat <- cor(TenYearCor)

#formatting matrix
corrplot(cordat, 
         method ="number",
         col = COL1("YlOrRd",200),
         type= "lower", 
         tl.col = "black",
         col.lim = c(-1,1))
```
`FIG_DecadalCor`

#Relationships between GHG and Outbreaks

```{r}
Torn<-BigDays.sfdfT%>%
  group_by(Year)%>%
  summarise(Totout_yr = n(),
            T_avgyear = mean(nT),
            totTorn = sum(nT),
            Tot_Cas = sum(GroupDayCas))

max(Torn$Totout_yr)
min(Torn$Totout_yr)
max(Torn$Tot_Cas)
min(Torn$Tot_Cas)
```

**10 Year Averages CO2 and Outbreak Relationship**
```{r}
p1 <- ggplot() +
  geom_bar(data = Torn, 
           aes(x=Year, y=Totout_yr*7), 
            stat="identity", 
            fill="grey40", 
            linewidth = 1) +
  geom_line(data = BigDays.sfdfT, 
            aes(x = Year, y = CO2_10year), 
            stat = "identity",
            color ="blue", 
            linewidth = 1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "blue")) +
  ylab(expression("Carbon Dioxide (in ppm)"))+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 450),
                     breaks = seq(0,450, 50), 
                     sec.axis = sec_axis(~./7, name = "Number of Clusters"))
p1
```

**10 year averages CH4 and Tornado Outbreaks**
```{r}
p2 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Totout_yr*30), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = CH4_10year), 
            color="purple3", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "purple3")) +
  ylab(expression("Methane (in ppb)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 2000),
                     breaks = seq(0,2000, 500), 
                     sec.axis = sec_axis(~./30, name = "Number of Clusters")) 
p2
```

**10 year averages N2O and Tornado Outbreaks**
```{r}
p4 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Totout_yr*6), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = N2O_10year), 
            color="deeppink", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "deeppink")) +
  ylab(expression("Nitrous Oxide (in ppb)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 350),
                     breaks = seq(0,350, 50), 
                     sec.axis = sec_axis(~./6, name = "Number of Clusters")) 
p4
```

**10 year averages SF6 and Tornado Outbreaks**
```{r}
p3 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Totout_yr/10), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = SF6_10year), 
            color="red2", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "red2")) +
  ylab(expression("Sulfar Hexafluoride (in ppt)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 10),
                     breaks = seq(0,10, 1), 
                     sec.axis = sec_axis(~.*10, name = "Number of Clusters")) 
p3
```

**10 year averages O3 and Tornado Outbreaks**
```{r}
p5 <- ggplot()+
  geom_bar(data = Torn, 
           aes(x=Year, y=Totout_yr/.5), 
           fill="grey40",
           stat ="identity",
           linewidth = 1)+
  geom_line(data =BigDays.sfdfT,
            aes(x = Year, y = O3_10year), 
            color = "goldenrod3", 
            stat = "identity",
            linewidth=  1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "goldenrod3")) +
  ylab(expression("Ozone (in DU)"))+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 140), 
                     breaks = seq(0,140, 20), 
                     sec.axis = sec_axis(~.*.5, name = "Number of Clusters")) 
```

**10 year averages AGGI and Tornado Outbreaks**
```{r}
p6 <- ggplot()+
  geom_bar(data=Torn,
           aes(x=Year, y=Totout_yr/50), 
           stat = "identity",
           fill="grey40", 
           linewidth=1)+
  geom_line(data = BigDays.sfdfT, 
            aes(x = Year, y = AGGI_10year), 
            stat = "identity",
            color = "green4", 
            linewidth = 1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "green4")) +
  ylab("Annual Greenhouse Gas Index")+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 1.5), 
                     breaks = seq(0,1.5,.2), 
                     sec.axis = sec_axis(~.*50, name = "Number of Clusters")) 
```

```{r}
grid.arrange(p1, p2, p3, p4, p5, p6,
             nrow=3,
             ncol=2, 
             top = textGrob(" ", 
                            gp=gpar(fontsize = 20)))
```
`FIG_DecTrends`

**10 Year Averages CO2 and Outbreak Relationship**
```{r}
p1 <- ggplot() +
  geom_bar(data = Torn, 
           aes(x=Year, y=Tot_Cas/15), 
            stat="identity", 
            fill="grey40", 
            linewidth = 1) +
  geom_line(data = BigDays.sfdfT, 
            aes(x = Year, y = CO2_1year), 
            stat = "identity",
            color ="blue", 
            linewidth = 1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "blue")) +
  ylab(expression("Carbon Dioxide (in ppm)"))+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 450),
                     breaks = seq(0,450, 50), 
                     sec.axis = sec_axis(~.*15, name = "Total Casualties"))
p1
```

**10 year averages CH4 and Tornado Outbreaks**
```{r}
p2 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Tot_Cas/3.5), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = CH4_10year), 
            color="purple3", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "purple3")) +
  ylab(expression("Methane (in ppb)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 2000),
                     breaks = seq(0,2000, 500), 
                     sec.axis = sec_axis(~.*3.5, name = "Total Casualties")) 
p2
```

**10 year averages N2O and Tornado Outbreaks**
```{r}
p4 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Tot_Cas/15), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = N2O_10year), 
            color="deeppink", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 20, color = "deeppink")) +
  ylab(expression("Nitrous Oxide (in ppb)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 500),
                     breaks = seq(0,500, 100), 
                     sec.axis = sec_axis(~.*15, name = "Total Casualties")) 
p4
```

**10 year averages SF6 and Tornado Outbreaks**
```{r}
p3 <- ggplot()+
  geom_bar(data = Torn,
           aes(x = Year, y = Tot_Cas/600), 
           fill ="grey40", 
           stat = "identity",
           linewidth = 1)+
  geom_line(data = BigDays.sfdfT,
            aes(x = Year, y = SF6_10year), 
            color="red2", 
            stat = "identity",
            linewidth = 1.5) +
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "red2")) +
  ylab(expression("Sulfar Hexafluoride (in ppt)")) +
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 10),
                     breaks = seq(0,10, 1), 
                     sec.axis = sec_axis(~.*600, name = "Total Casualties")) 
p3
```

**10 year averages O3 and Tornado Outbreaks**
```{r}
p5 <- ggplot()+
  geom_bar(data = Torn, 
           aes(x=Year, y=Tot_Cas/50), 
           fill="grey40",
           stat ="identity",
           linewidth = 1)+
  geom_line(data =BigDays.sfdfT,
            aes(x = Year, y = O3_10year), 
            color = "goldenrod3", 
            stat = "identity",
            linewidth=  1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "goldenrod3")) +
  ylab(expression("Ozone (in DU)"))+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 140), 
                     breaks = seq(0,140, 20), 
                     sec.axis = sec_axis(~.*50, name = "Total Casualties")) 
```

**10 year averages AGGI and Tornado Outbreaks**
```{r}
p6 <- ggplot()+
  geom_bar(data=Torn,
           aes(x=Year, y=Tot_Cas/5000), 
           stat = "identity",
           fill="grey40", 
           linewidth=1)+
  geom_line(data = BigDays.sfdfT, 
            aes(x = Year, y = AGGI_10year), 
            stat = "identity",
            color = "green4", 
            linewidth = 1.5)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, 
                                 hjust=1, 
                                 size = 14),
         axis.text.y = element_text(size = 14), 
         axis.title.x = element_text(size = 16),
         axis.title.y.right = element_text(size = 16, color = "black"),
         axis.title.y.left = element_text(size = 16, color = "green4")) +
  ylab("Annual Greenhouse Gas Index")+
  scale_x_continuous(expand = c(0,0),
                     limits=c(1993,2023), 
                     breaks=seq(1994,2022,4))+
  scale_y_continuous(expand = c(0, 0), 
                     limits=c(0, 1.5), 
                     breaks = seq(0,1.5,.2), 
                     sec.axis = sec_axis(~.*5000, name = "Total Casualties")) 
```

```{r}
grid.arrange(p1, p2, p3, p4, p5, p6,
             nrow=3,
             ncol=2, 
             top = textGrob(" ", 
                            gp=gpar(fontsize = 20)))
```
FIG_DecTrends2

```{r}
max(BigDays.sfdfT$CO2_1year)
min(BigDays.sfdfT$CO2_1year)
max(BigDays.sfdfT$CO2_5year)
min(BigDays.sfdfT$CO2_5year)
max(BigDays.sfdfT$CO2_10year)
min(BigDays.sfdfT$CO2_10year)

max(BigDays.sfdfT$CH4_1year)
min(BigDays.sfdfT$CH4_1year)
max(BigDays.sfdfT$CH4_5year)
min(BigDays.sfdfT$CH4_5year)
max(BigDays.sfdfT$CH4_10year)
min(BigDays.sfdfT$CH4_10year)

max(BigDays.sfdfT$SF6_1year)
min(BigDays.sfdfT$SF6_1year)
max(BigDays.sfdfT$SF6_5year)
min(BigDays.sfdfT$SF6_5year)
max(BigDays.sfdfT$SF6_10year)
min(BigDays.sfdfT$SF6_10year)

max(BigDays.sfdfT$N2O_1year)
min(BigDays.sfdfT$N2O_1year)
max(BigDays.sfdfT$N2O_5year)
min(BigDays.sfdfT$N2O_5year)
max(BigDays.sfdfT$N2O_10year)
min(BigDays.sfdfT$N2O_10year)

max(BigDays.sfdfT$O3_1year)
min(BigDays.sfdfT$O3_1year)
max(BigDays.sfdfT$O3_5year)
min(BigDays.sfdfT$O3_5year)
max(BigDays.sfdfT$O3_10year)
min(BigDays.sfdfT$O3_10year)

max(BigDays.sfdfT$AGGI_1year)
min(BigDays.sfdfT$AGGI_1year)
max(BigDays.sfdfT$AGGI_5year)
min(BigDays.sfdfT$AGGI_5year)
max(BigDays.sfdfT$AGGI_10year)
min(BigDays.sfdfT$AGGI_10year)
```



######################
Significance Testing
######################

```{r}
dat <- BigDays.sfdfT %>%
  group_by(Year) %>%
  summarize(numClus = n(),
            numTorn = sum(nT),
            numCas = sum(GroupDayCas),
            CO2_10year = max(CO2_10year),
            CH4_10year = max(CH4_10year), 
            O3_10year = max(O3_10year),
            AGGI_10year = max(AGGI_10year))
```

```{r}
cor.test(dat$numClus, dat$Year, method = "pearson")
cor.test(dat$numTorn, dat$Year, method = "pearson")
cor.test(dat$numCas, dat$Year, method = "pearson")

cor.test(dat$CO2_10year, dat$Year, method = "pearson")
cor.test(dat$CH4_10year, dat$Year, method = "pearson")
cor.test(dat$O3_10year, dat$Year, method = "pearson")
cor.test(dat$AGGI_10year, dat$Year, method = "pearson")

cor.test(dat$numClus, dat$CO2_10year, method = "pearson")
cor.test(dat$numClus, dat$CH4_10year, method = "pearson")
cor.test(dat$numClus, dat$O3_10year, method = "pearson")
cor.test(dat$numClus, dat$AGGI_10year, method = "pearson")

cor.test(dat$numTorn, dat$CO2_10year, method = "pearson")
cor.test(dat$numTorn, dat$CH4_10year, method = "pearson")
cor.test(dat$numTorn, dat$O3_10year, method = "pearson")
cor.test(dat$numTorn, dat$AGGI_10year, method = "pearson")

cor.test(dat$numCas, dat$CO2_10year, method = "pearson")
cor.test(dat$numCas, dat$CH4_10year, method = "pearson")
cor.test(dat$numCas, dat$O3_10year, method = "pearson")
cor.test(dat$numCas, dat$AGGI_10year, method = "pearson")
```

Null: There is no difference (variables not related)
Alt: They are related
```{r}
cor.test(BigDays.sfdfT$maxCAPE, BigDays.sfdfT$Year, method = "pearson")
cor.test(BigDays.sfdfT$minCIN, BigDays.sfdfT$Year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$Year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_shallow, BigDays.sfdfT$Year, method = "pearson")

cor.test(BigDays.sfdfT$maxCAPE, BigDays.sfdfT$CO2_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxCAPE, BigDays.sfdfT$CH4_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxCAPE, BigDays.sfdfT$O3_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxCAPE, BigDays.sfdfT$AGGI_10year, method = "pearson")

cor.test(BigDays.sfdfT$minCIN, BigDays.sfdfT$CO2_10year, method = "pearson")
cor.test(BigDays.sfdfT$minCIN, BigDays.sfdfT$CH4_10year, method = "pearson")
cor.test(BigDays.sfdfT$minCIN, BigDays.sfdfT$O3_10year, method = "pearson")
cor.test(BigDays.sfdfT$minCIN, BigDays.sfdfT$AGGI_10year, method = "pearson")

cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$CO2_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$CH4_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$O3_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$AGGI_10year, method = "pearson")

cor.test(BigDays.sfdfT$maxBS_shallow, BigDays.sfdfT$CO2_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_shallow, BigDays.sfdfT$CH4_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_shallow, BigDays.sfdfT$O3_10year, method = "pearson")
cor.test(BigDays.sfdfT$maxBS_shallow, BigDays.sfdfT$AGGI_10year, method = "pearson")
```

```{r}
test <- BigDays.sfdfT %>%
  group_by(Year) %>%
  summarize(CAPE = mean(maxCAPE, na.rm = TRUE),
            CIN = mean(minCIN, na.rm = TRUE),
            DLBS = mean(maxBS_deep, na.rm = TRUE),
            SLBS = mean(maxBS_shallow, na.rm = TRUE))
```

```{r}
plot_CAPE <- ggplot(data = test, 
                    aes(x = Year, y =CAPE)) + 
  geom_bar(stat = "identity",
           fill = "purple4",
           lwd = 1) +
  geom_smooth(method = lm, 
               color = "black", 
              lwd = 1,
              se = FALSE) +
  #ggtitle("CAPE")+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, 
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year") +
  ylab("Convective Available Potential Energy [J/kg]") +
  scale_x_continuous(expand = c(0,0),
                     limits = c(1993, 2023),
                     breaks = seq(1994, 2022, 2)) +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,3000,250), 
                     breaks = seq(0,3000,250))
plot_CAPE
```

```{r}
plot_CIN <- ggplot(data = test, 
                    aes(x = Year, y =CIN*-1)) + 
  geom_bar(stat = "identity",
           fill = "darkgreen",
           lwd = 1) +
  geom_smooth(method = lm, 
               color = "black", 
              lwd = 1,
              se = FALSE) +
  #ggtitle("CAPE")+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, 
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year") +
  ylab("Convective Inhibition [J/kg]") +
  scale_x_continuous(expand = c(0,0),
                     limits = c(1993, 2023),
                     breaks = seq(1994, 2022, 2)) +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,200,20), 
                     breaks = seq(0,200,20))
plot_CIN
```

```{r}
plot_DLBS <- ggplot(data = test, 
                    aes(x = Year, y =DLBS)) + 
  geom_bar(stat = "identity",
           fill = "dodgerblue",
           lwd = 1) +
  geom_smooth(method = lm, 
               color = "black", 
              lwd = 1,
              se = FALSE) +
  #ggtitle("CAPE")+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, 
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year") +
  ylab("Deep-Layer Shear [m/s]") +
  scale_x_continuous(expand = c(0,0),
                     limits = c(1993, 2023),
                     breaks = seq(1994, 2022, 2)) +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,40,5), 
                     breaks = seq(0,40,5))
plot_DLBS
```

```{r}
plot_SLBS <- ggplot(data = test, 
                    aes(x = Year, y =SLBS)) + 
  geom_bar(stat = "identity",
           fill = "darkred",
           lwd = 1) +
  geom_smooth(method = lm, 
               color = "black", 
              lwd = 1,
              se = FALSE) +
  #ggtitle("CAPE")+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, 
                                  size = 20), 
        plot.margin = margin(0.5,0.5,0.5,0.5, "cm"), 
        axis.text.x = element_text(angle=45, 
                                   hjust=1,
                                   size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16)) +
  xlab("Year") +
  ylab("Shallow-Layer Shear [m/s]") +
  scale_x_continuous(expand = c(0,0),
                     limits = c(1993, 2023),
                     breaks = seq(1994, 2022, 2)) +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,25,5), 
                     breaks = seq(0,2540,5))
plot_SLBS
```

```{r}
ggarrange(plot_CAPE, plot_CIN, plot_DLBS, plot_SLBS)
```

```{r}
BigDays.sfdfT %>%
  group_by(Mo) %>%
  summarize(numClus =n())
  
```

```{r, eval = FALSE}
library(tidyverse)
df <- All_Tornadoes %>% extract(geometry, c('slon', 'slat'), '\\((.*), (.*)\\)', convert = TRUE) 
lon_lat <- df[,28:29]
All_Tornadoes <- cbind(All_Tornadoes, lon_lat)
```

```{r, eval = FALSE}
BigDayTornadoes <- All_Tornadoes %>%
   mutate(ID = paste0(gsub("-", "", cDate), groupNumber))
BigDays.sfdfT <- BigDays.sfdfT %>%
   mutate(ID = paste0(gsub("-", "", cDate), groupNumber))

BigDayTornadoes <- BigDayTornadoes %>%
  filter(ID %in% BigDays.sfdfT$ID)
```

 New Addition to the Data Files
```{r, eval = FALSE}
#save(All_Tornadoes, BigDayCentroids.df, BigDays.sfdfT, BigDayTornadoes, Groups.sfdfT, file = "TornadoOutbreaks.RData")
#save(All_Tornadoes, BigDayCentroids.df, BigDays.sfdfT, BigDayTornadoes, Groups.sfdfT, file = "TornadoOutbreaks_Backup.RData")
```

```{r}
ewbrks <- seq(-120, -70,10)
nsbrks <- seq(25,50,5)
ewlbls <- unlist(lapply(ewbrks, function(x) ifelse(x < 0, paste(x*-1, "°W"), ifelse(x > 0, paste(x, "°E"),x))))
nslbls <- unlist(lapply(nsbrks, function(x) ifelse(x < 0, paste(x, "°S"), ifelse(x > 0, paste(x, "°N"),x))))
```


```{r}
BigDayCentroids.df %>%
  group_by(st) %>%
  summarize(numClus = n())

names <- array(month.name)

p <- (ggplot(stateBorders) +
  geom_sf(fill = NA, color = "gray70") + 
  stat_density2d(aes(x = slon, y = slat, fill = ..level..), alpha = .5, h = 5,
                 geom = "polygon", data = BigDayTornadoes) +
  facet_wrap(~mo, ncol = 3, labeller = labeller(mo = 
                    c("1" = "January",
                      "2" = "February",
                      "3" = "March",
                      "4" = "April",
                      "5" = "May",
                      "6" = "June",
                      "7" = "July",
                      "8" = "August",
                      "9" = "September",
                      "10" = "October",
                      "11" = "November",
                      "12" = "December"))) +
    scale_fill_distiller(palette = "YlOrRd", direction  = 1) +
 scale_x_continuous(breaks = ewbrks, labels = ewlbls) +
  scale_y_continuous(breaks = nsbrks, labels = nslbls) +
  labs(x = "Longitude", y = "Latitude") +#, fill = "Density") +
  #scale_fill_gradient(low = "#FCBBA1", high = "#99000D") + 
  theme_minimal())

p <- p + theme(axis.text.x = element_text(angle = 45, margin = margin(t = 20)), axis.text.y = element_text(margin = margin(r = 20)), axis.title.y = element_text( margin = margin(r=20)), strip.text = element_text(size=16), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), legend.position="none")

p
```

```{r}
p <- (ggplot(stateBorders) +
  geom_sf(fill = NA, color = "gray70") + 
  stat_density2d(aes(x = Lon, y = Lat, col = maxCAPE), alpha = .5, h = 5,
                 geom = "polygon", data = BigDays.sfdfT) +
    scale_fill_distiller(palette = "YlOrRd", direction  = 1) +
 scale_x_continuous(breaks = ewbrks, labels = ewlbls) +
  scale_y_continuous(breaks = nsbrks, labels = nslbls) +
  labs(x = "Longitude", y = "Latitude") +#, fill = "Density") +
  #scale_fill_gradient(low = "#FCBBA1", high = "#99000D") + 
  theme_minimal())

p <- p + theme(axis.text.x = element_text(angle = 45, margin = margin(t = 20)), axis.text.y = element_text(margin = margin(r = 20)), axis.title.y = element_text( margin = margin(r=20)), strip.text = element_text(size=16), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), legend.position="none")

p
```

```{r}
library(gstat)
library(tidyverse)
library(stars) 
library(purrr)
```

```{r}
k <- variogram(maxCAPE~1, BigDays.sfdfT, cutoff = 1500, width = 50)

plot(k, threshold = 4) #linear variogram

library(kableExtra)
fit.lin <- vgm(1, 'Lin', 0)
```
Grid of the US: 
```{r}
ch_grid <- stateBorders %>% 
   st_as_sf() %>% 
   st_make_grid(cellsize = 1, what = "polygons")

  ch_grid %>% st_set_crs(st_crs(BigDays.sfdfT)) 
  grd <- st_transform(ch_grid, crs = st_crs(BigDays.sfdfT))
```

```{r}
coords <- BigDayCentroids.df %>%
    mutate(lat = unlist(map(BigDayCentroids.df$geometry,1)),
           long = unlist(map(BigDayCentroids.df$geometry,2)))

coords <- coords[,66:67]
BigDays.sfdfT <- cbind(BigDays.sfdfT,coords)

#BigDays.sfdfT <- as.data.frame(BigDays.sfdfT)
#coordinates(BigDays.sfdfT) = ~lat+long

BigDays.sfdfT %>% st_set_crs(st_crs(grd)) 
BigDays.sfdfT <- st_transform(BigDays.sfdfT, crs = st_crs(grd))

CAPE.lin <- krige(maxCAPE~1, BigDays.sfdfT, grd, model=fit.lin)
spplot(CAPE.lin["var1.pred"], main = "ordinary kriging predictions")
```

```{r}
US_geom <-as.data.frame(ch_grid)
CAPE_Krig <- cbind(CAPE.lin, US_geom)

tm_shape(CAPE_Krig, 
         is.master = TRUE, 
         projection = merc) + 
  tm_fill(col="var1.pred", alpha = .5, 
              border.col = "black", 
              lwd = 1.5) +
tm_shape(stateBorders, 
         projection = merc) +
  tm_borders(col ="gray70", 
             alpha = 1) +
  tm_compass(text.size = 1.25, size = 3,
             lwd = 2, 
             color.dark = "gray70") +
  tm_scale_bar(width = .25, 
               text.size = 1.25, 
               lwd = 1.75,  
               color.dark = "gray70") +
  tm_layout(attr.position = c("right","bottom"), 
            inner.margins = c(.01, .01, .01, .01))
```


https://r-spatial.github.io/gstat/reference/krige.html 
https://www.cfholbert.com/blog/esda-kriging/

```{r}
test <- CAPE.lin
test<-as.data.frame(CAPE.lin)
coordinates(test) = ~x+y
spplot(test["var1.pred"])
```



```{r}
library(purrr)


ch_grid <- stateBorders %>% 
   st_as_sf() %>% 
   st_make_grid(cellsize = 0.1, what = "polygons")

```

```{r}
lon_lat <- df[,28:29]
All_Tornadoes <- cbind(All_Tornadoes, lon_lat)



ch_grid <- stateBorders %>% 
  st_as_sf() %>% 
  mutate(bbox = map(geometry, st_bbox),
         grid = map(geometry, ~ st_make_grid(., cellsize = 0.1, what = "polygons")))
```

