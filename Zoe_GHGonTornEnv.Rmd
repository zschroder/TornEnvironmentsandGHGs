...
x
```
Cat       NumClus   NumCas    NumTorns
10 - 29   711       9106      11410
30 - 59   142       7804      5607
60 - 89   29        4785      2075
90+       8         3685      888
...
```{r}
grid.arrange(CO2_plot, CH4_plot, SF6_plot, N2O_plot, O3_plot, AGGI_plot, nrow = 2)
```
...
GHGVars <- data.frame(CO2 =  BigDays.sfdfT$CO2_10year, 
                      ...
                      AirT = BigDays.sfdfT$max_globalairtemp,
                      monATL = BigDays.sfdfT$monthlyATLSST,
                      monGAK = BigDays.sfdfT$monthlyGAKSST,
                      monNino = as.numeric(BigDays.sfdfT$monthlyElNino34),
                      monSIE = BigDays.sfdfT$monthlySIE,
                      NAO30 = BigDays.sfdfT$NAO_30day,
                      PNA30 = BigDays.sfdfT$PNA_30day,
                      MJO30 = BigDays.sfdfT$MJO_30day)
```
...
Initial Model:
```{r}
DLBS_m1 <- lm(maxBS_deep ~ CO2_10year + O3_10year + max_globalairtemp + monthlyATLSST + monthlyGAKSST + as.numeric(monthlyElNino34) + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year, data = BigDays.sfdfT)
summary(DLBS_m1)
```
...
```{r}
cor.test(BigDays.sfdfT$maxBS_deep, BigDays.sfdfT$Year)
```
p-value = 0.05364

```{r}
library(lme4)
DLBS_m2 <- lmer(maxBS_deep ~ CO2_10year + O3_10year + max_globalairtemp + monthlyATLSST + monthlyGAKSST + as.numeric(monthlyElNino34) + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m2)
```

```{r}
DLBS_m3 <- lmer(maxBS_deep ~ CO2_10year + O3_10year + max_globalairtemp + monthlyATLSST + monthlyGAKSST + as.numeric(monthlyElNino34) + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m3)
```


```{r}
DLBS_m3 <- lmer(maxBS_deep ~ CO2_10year + O3_10year + max_globalairtemp + as.numeric(monthlyElNino34) + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m3)
```

```{r}
DLBS_m4 <- lmer(maxBS_deep ~ CO2_10year + O3_10year  + as.numeric(monthlyElNino34) + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m4)
```

```{r}
DLBS_m5 <- lmer(maxBS_deep ~ CO2_10year + O3_10year  + monthlySIE + NAO_30day + PNA_30day + MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m5)
```

```{r}
AIC(DLBS_m1, DLBS_m2, DLBS_m3, DLBS_m4, DLBS_m5)
```
...
library("sjstats")
performance::r2(DLBS_model)
```
This model explains 40.4% of the variation of DLBS.
